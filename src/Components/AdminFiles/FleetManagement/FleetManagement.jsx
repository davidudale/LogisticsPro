...
const buildDriverNameOptions = (fleetDriverSnapshot, userDriverSnapshot) => {
  const names = new Set();

  fleetDriverSnapshot.docs.forEach((item) => {
    const fullName = item.data()?.fullName?.trim();
    if (fullName) names.add(fullName);
  });

  userDriverSnapshot.docs.forEach((item) => {
    const data = item.data();
    if (normalizeRole(data.role) !== ROLE.DRIVER) return;
...
const FleetManagement = () => {
  ...
  useEffect(() => {
    let latestFleetSnapshot = null;
    let latestUserSnapshot = null;

    const syncDriverOptions = () => {
      if (!latestFleetSnapshot || !latestUserSnapshot) return;
      setDriverOptions(buildDriverNameOptions(latestFleetSnapshot, latestUserSnapshot));
    };

    const unsubscribeFleetDrivers = onSnapshot(collection(db, DRIVERS_COLLECTION), (snapshot) => {
      latestFleetSnapshot = snapshot;
      syncDriverOptions();
    }, (error) => {
      console.error("[Firestore][FleetManagement] Failed watching collection", {
        collection: DRIVERS_COLLECTION,
        error,
      });
      toast.error(error?.message || "Failed to load fleet driver options.");
    });

    const unsubscribeUserDrivers = onSnapshot(
      ...
      (error) => {
        ...
        toast.error(error?.message || "Failed to load driver user options.");
      },
    );

    return () => {
      unsubscribeFleetDrivers();
      unsubscribeUserDrivers();
    };
  }, []);
