...
const PendingQuotations = () => {
  ...
  const deleteQuotation = async () => {
    ...
    setBusyRow("delete-quotation");
    try {
      await deleteDoc(doc(db, "Quotations", deleteTarget.id));
  ...
  const filteredQuotations = useMemo(() => {
    ...
    if (!query) {
      return sortByNewest(visibleQuotations);
    }

    return sortByNewest(visibleQuotations.filter((quotation) => {
